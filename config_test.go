...
# A comment at the beginning of the line
{
  ...
  "files": [
	...
	defaultDeadTime, _ := time.ParseDuration(default_FileConfig_DeadTime)
	expected := Config{
		...
		Files: []FileConfig{{
			...
			DeadTime: default_FileConfig_DeadTime,
...
func TestFinalizeConfig(t *testing.T) {
	...
	if config.Network.Timeout != default_NetworkConfig_Timeout {
		t.Fatalf("Expected FinalizeConfig to default timeout to %d, got %d instead", default_NetworkConfig_Timeout, config.Network.Timeout)
	}
